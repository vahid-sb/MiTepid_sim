...
Studying the effects of various Suppression Strategies on the spread of
    SARS-CoV-2 in the population of Germany.
...
if __name__ == '__main__':
    ...
    dir_save_plots = Path(Path(__file__).resolve().parent, 'OUTPUTS_submitted', 'Uncontained_then_single_policy')
    ...
    filesave_I = Path(dir_save_plots, model_type + '_I_AGG_ALL_policies' + country +'.png')
    filesave_R = Path(dir_save_plots, model_type + '_R_AGG_ALL_policies' + country +'.png')

    # %% plot aggregate plots to compare all policies.
    t = np.arange(0, t_end+.01, step=0.1)
    list_all_policies = ['Uncontained']
    list_all_policies.extend(['New Policy']*(len(policy_list)-1))
