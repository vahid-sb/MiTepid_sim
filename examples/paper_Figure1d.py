...
# -*- coding: utf-8 -*-
"""
Created on Fri Oct 23 22:32:19 2020

@author: vbokharaie

Studying what happens if T_Infectious varies.
T_Infectious is the average time-period in individuals infected with SARS-CoV-2 are infectious.
...
if __name__ == '__main__':
    ...
    dir_save_plots = Path(Path(__file__).resolve().parent, 'OUTPUTS_submitted', 'Compare_T_Infectious_Uncontained')
    ...
    list_sol_I = []
    list_sol_R = []
    list_labels = ['Ti ' + str(x) + ' days' for x in np.arange(10) + 5]
    for T_I in np.arange(10) + 5:
        print('Infectious time = ', T_I)
        model_type = 'SEIR'
        file_Bopt = Path('B_opt_SEIR_Ti_' + str(T_I) + '_R0_2.95.mat')
    ...
    filesave_I = Path(dir_save_plots, model_type + '_' + country +  '_I_AGG_ALL_T_I.png')
    filesave_R = Path(dir_save_plots, model_type + '_' + country +  '_R_AGG_ALL_T_I.png')
    ...
    filesave_txt = Path(dir_save_plots, model_type + country + '_variable_T_I_eventual_I_R.txt')
    max_I = np.max(arr_sol_I, axis=0) * 100
    end_R = arr_sol_R[-1, :] * 100
    with open(filesave_txt, 'w') as my_text:
        my_text.write('\n %14s &  %6s   &  %6s '%('T_i  ', 'end_R', 'max_I'))
        for idx, Ti in enumerate(list_labels):
            my_text.write('\n %14s &  $%2.2f\%%$ &  $%2.2f\%%$ \\\\ \hline'%(Ti, end_R[idx], max_I[idx]))
