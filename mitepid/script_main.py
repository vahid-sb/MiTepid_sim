...
if __name__ == '__main__':
    ...
    dir_save_plots_main = Path(Path(__file__).resolve().parent, 'output_main2')
    ...
    #
    if model_type == 'SIS':
        file_selected = Path(dir_source_mat, 'Optimised_B', 'B_opt_SIS_0602T191117.mat')
    elif model_type == 'SIR':
        file_selected = Path(dir_source_mat, 'Optimised_B', 'B_opt_SIR_0602T181338.mat')
    elif model_type == 'SEIR':
        file_selected = Path(dir_source_mat, 'Optimised_B', 'B_opt_SEIR_0602T190324.mat')

    B_opt_selected = get_B_policy(file_selected, country, 'Uncontained', )
    Ng = B_opt_selected.shape[0]
    Gamma = load_mat(file_selected, 'Gamma')
    Sigma = load_mat(file_selected, 'Sigma')
